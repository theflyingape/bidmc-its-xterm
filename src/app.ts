...
dns.lookup(config.host, (err, addr, family) => {
    ...
    wss.on('connection', (browser, req) => {
        ...
        term.onExit(() => {
            if (term.client) {
                syslog.note(`Exit app PID: ${pid} CLIENT: ${term.client}`)
                delete sessions[pid]
                pid = 0
                browser.close()
            }
            else {
                syslog.warn(`Exit orphaned app PID: ${pid}`)
            }
        })
        ...
        browser.on('message', (msg) => {
            try {
                ...
                if (pid) {
                    syslog.warn(`Aborted ws -> pty I/O from browser CLIENT: ${term.client} to app PID: ${pid} error:`, ex.message)
                    browser.close()
                }
            }
        })
        ...
        browser.on('close', () => {
            ...
            if (pid > 1) try {
                ...
                syslog.note(`Terminated app PID: ${pid} CLIENT: ${term.client}`)
            } catch (ex) {
                syslog.error(`?FATAL browser CLIENT: ${term.client} close event on PID: ${pid} "${ex}"`)
            }
        })
    })
})
